...
def execute_posting(self, task_id: str) -> None:
    ...
    try:
        ...
        if gen_content.media and gen_content.media[0].is_generated:
            ...
            imgUrl = media.img_url or media.storage_path  # Prefer img_url if available
        ...
        for sel in task.platform_selections:
            ...
            try:
                ...
                latency_ms = int((time.time() - start_time) * 1000)
                attempt.status = AttemptStatus.permanent_failure  # Assume permanent; customize for transient (e.g., rate limit)
                attempt.latency_ms = latency_ms
                ...
                error_log = _handle_post_failure(
                    ...
                    error_code="POSTING_ERROR",  # Customize per platform if needed
